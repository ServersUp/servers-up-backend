...
Discord commands (`/subscribe`, `/games`, `/servers`, `/status`, autocomplete, notification labels) use **`server-mapping.json`** in S3 (`CONFIG_BUCKET` / `SERVER_MAPPING_PATH`). Each game has a `provider`, and each server has `region` and `identifier` (for Battle.net, the connected realm ID). `/status` resolves game/server through the mapping, then reads live status from the status DynamoDB table (`DDB_TABLE_NAME` on the bot Lambda).
