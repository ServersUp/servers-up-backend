...
Discord commands (`/subscribe`, `/games`, autocomplete, notification labels) use **`server-mapping.json`** in S3 (`CONFIG_BUCKET` / `SERVER_MAPPING_PATH`). Each game has a `provider`, and each server has `region` and `identifier` (for Battle.net, the connected realm ID).
