...
func TestAutocompleteServers_requiresGame(t *testing.T) {
	t.Parallel()
	m := servermap.Mapping{Games: map[string]servermap.Game{
		"wow": {Servers: map[string]servermap.Server{"illidan": {}}},
	}}
	choices, err := autocompleteServers(m, "", "ill", 25)
...
func TestAutocompleteServers_listsServers(t *testing.T) {
	t.Parallel()
	m := servermap.Mapping{Games: map[string]servermap.Game{
		"wow": {Servers: map[string]servermap.Server{
			"illidan": {},
			"area-52": {},
		}},
	}}
